...
SRT / VTT では、字幕が動画画面からはみ出しにくいように自動整形します。

- 1行あたりの目安: 全角18文字
- 不自然な分割になる場合は、自然な切れ目を優先して全角24文字相当まで許容
- 最大行数: 2行
- 長すぎる場合: 字幕 cue を複数に分割
- 句読点、文字種、断片の長さから自然な切れ目を推定して改行
- 文字種と断片の長さを見て、短い語尾だけが次行に残る分割を抑制
- cue や行末の不要な読点 `、` は表示から削除
- 3行に分かれる場合は、最後の短い行が単独字幕にならないように配置
- 短すぎる字幕は、可能な場合に前後の字幕へ吸収
- 近い短文字幕は、2行以内・最大6秒以内で結合
- 字幕終了時に最大0.45秒の読了マージンを追加
- 分割時刻は word timestamp を優先して決定
